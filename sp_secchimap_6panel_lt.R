...
map1<- map1 + theme(legend.position = "none") +
  geom_segment(x = -96.135058, y = 33.646397, xend = -96.135981, yend = 33.655287,lwd=2,color=gray(0.5))+ # dam location
...
map2<- map2 + theme(legend.position = "none") +
  geom_segment(x = -97.1910, y = 31.576152, xend = -97.2136, yend = 31.594505,lwd=2,color=gray(0.5))+
...
map3<- map3 + theme(legend.position = "none") +
  geom_segment(x = -99.0031, y = 31.8352, xend = -99.0002, yend = 31.8421,lwd=2,color=gray(0.5))+
...
map4<- map4 + theme(legend.position = "none") +
  geom_segment(x = -99.6684, y = 31.4970, xend = -99.6640, yend = 31.5025,lwd=2,color=gray(0.5))+
...
map5<- map5 + theme(legend.position = "none") +
  geom_segment(x = -103.9121, y = 31.8962, xend = -103.9080, yend = 31.9069,lwd=2,color=gray(0.5))+
...
map6<- map6 + theme(legend.position = "none") +
  geom_segment(x = -98.3539, y = 33.7653, xend = -98.3753, yend = 33.7657,lwd=2,color=gray(0.5))+
...
plots <- list(map1, map2, map3, map4, map5, map6)

# Suppress legends in all plots except for the first one
plots_no_legend <- lapply(plots, function(p) p + theme(legend.position = "none"))

# Combine the plots into a patchwork layout and place the legend at the bottom
combined_plot <- (wrap_plots(plots_no_legend, ncol = 3)) + 
  plot_layout(guides = "collect") & 
  theme(legend.position = "right")

combined_plot_bot<- (wrap_plots(plots_no_legend, ncol = 3)) + 
  plot_layout(guides = "collect") & 
  theme(legend.position = "bottom")

# Display the final combined plot
combined_plot
ggsave("combo_logturbmap.png", combined_plot,width=8,height=5)


combined_plot_bot
ggsave("combo_logturbmap_bot.png", combined_plot_bot,width=8,height=8)

# Combine plots into a single layout with grid.arrange
# Does this panel order make sense? 
plots <- arrangeGrob(map1, map2, map3,
                     map4, map5, map6,
                     ncol = 3)

combo_turb_lomarg <- grid.arrange(plots, legend, ncol = 2, widths = c(4, 1))
ggsave("combo_logturbmap_og.png", combo_turb_lomarg,width=9,height=8)
