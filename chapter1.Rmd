...
sdd_stations<-sample_ysi_secchi %>% filter(!system =="Lake Brownwood" & !system =="Lake Waco") %>% 
...
First we want to see the relationship between sensor turbidity and NDTI calculated along the boat path.
...
s2crasr %>% filter(dwl>500) %>% ggplot(aes(y=dwl,x=ndti)) +
  geom_point(aes(color = system)) +
  #geom_smooth(method = "lm", formula = y ~ x, color = "black") +
  stat_poly_eq(formula = y ~ x,
               aes(label = paste(after_stat(rr.label))),
